...
- ✅ Ett spel utan den guidade starten: från wizardens steg 1 med bara namn och spelarantal till ett tomt spel i editorn — inga kort, inga fält, tomma sidor, receptets bord — genom samma `POST /projects` som den guidade vägen (E3, L14).
...
- ✅ Regelboken: versionerat dokument med referenser till zoner och kort, skrivet i fliken "Regler", läst vid bordet och på telefonen, och tryckt som häfte i A5 genom samma renderare som korten; bilder bor i spelets egna tillgångar och mäts i millimeter mot häftets spalt på varje yta (B7).
