...
export function Switcher({ variants, current, onChange }: { variants: Variant[]; current: string; onChange(key: string): void }) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: 12, left: '50%', transform: 'translateX(-50%)', zIndex: 9999, display: 'flex', alignItems: 'center', gap: 10, padding: '6px 10px', borderRadius: 999, background: '#111', color: '#fff', boxShadow: '0 6px 24px rgba(0,0,0,.4)', fontSize: 12, fontFamily: 'ui-monospace, monospace' }}>
...
const btn: React.CSSProperties = { background: '#333', color: '#fff', border: 0, borderRadius: 999, width: 24, height: 24, cursor: 'pointer' }
