...
import { CardFace, Sym, tokensOf } from './symbol-shared.js'
  ...
  return (
    <div className="byd-proto-sym-b">
      <section className="byd-proto-cell">
        ...
        {raw ? (
          ...
          <div className="byd-proto-chipbox" aria-label="Korttext">
            <Rendered text={text} tokens={tokens} inkAt={inkAt} openAt={openAt} onOpen={setOpenAt} />
          </div>
        )}
        ...
        {openAt !== null && (
          <div className="byd-proto-pop" role="dialog" aria-label="Färg">
            <header>
      ...
      <aside className="byd-proto-preview">
        <h2>Kortet</h2>
        <CardFace title="Drake" text={text} ink={() => '#1c1c1c'} />
        <p className="byd-proto-hint">Kortet ovan ritas av samma kompilator, med färgerna nedan inlagda på varje bruk.</p>
...
function Rendered({ text, tokens, inkAt, openAt, onOpen, flat = false }: { text: string; tokens: ReturnType<typeof tokensOf>; inkAt(at: number): string; openAt: number | null; onOpen(at: number | null): void; flat?: boolean }) {
  ...
  for (const tk of tokens) {
    ...
    out.push(
      flat ? (
        ...
        <button key={tk.at} type="button" className="byd-proto-chip" aria-pressed={openAt === tk.at} onClick={() => onOpen(openAt === tk.at ? null : tk.at)} aria-label={`Färga ${tk.name}`}>
          {sym}
        </button>
      ),
    )
