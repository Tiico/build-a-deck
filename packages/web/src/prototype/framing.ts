...
export function windowFor(piece: Piece, rule: Rule, nudge: Nudge = NO_NUDGE): Window {
  const m = piece.motif
  // The window that gives the motif exactly `fill` of the height, then widened to the frame.
  const h = (m.h / rule.fill) / nudge.zoom
  const w = h * ratio
  const cx = m.x + m.w / 2
...
export function laidOut(piece: Piece, win: Window, boxW: number): { width: number; left: number; top: number } {
  const scale = boxW / win.w
  return { width: piece.w * scale, left: -win.x * scale, top: -win.y * scale }
}

// How large the drawing is drawn, as a share of the window's height. The rule says what this
// should be; this says what it is, which is the number a deck is judged on.
export const drawnAt = (piece: Piece, win: Window): number => piece.motif.h / win.h
