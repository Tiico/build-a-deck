...
export function whoDecides(view: Snapshot, proposal: RewindProposal): string {
  const others = view.seats.filter((s) => s.id !== proposal.by && s.name !== null).map((s) => s.name ?? s.id)
  const last = others.at(-1)
  if (last === undefined) return 'någon annan'
  return others.length === 1 ? last : `${others.slice(0, -1).join(', ')} eller ${last}`
}
...
export function whereTo(view: Snapshot, proposal: RewindProposal, activity: readonly Activity[]): string {
  const first = activity.find((l) => l.seq > proposal.toSeq && !l.intent.v.startsWith('rewind.'))
  return first ? `före ”${describeActivity(first, view)}”` : `vid drag ${proposal.toSeq}`
}
