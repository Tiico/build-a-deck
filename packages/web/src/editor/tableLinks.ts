...
export const onlineUrl = (session: string, server: string | null, seat: SeatId, owner = false, t?: T): string =>
  to('/online', new URLSearchParams({ session, seat, name: designerName(t), ...(owner ? { owner: '1' } : {}) }), server)
export const observeUrl = (session: string, server: string | null, owner = false, t?: T): string =>
