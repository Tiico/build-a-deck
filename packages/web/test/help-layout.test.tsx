...
const css = ['src/help.css', 'src/help-box.css', 'src/editor/editor.css', 'src/buttons.css', 'src/a11y.css', 'src/rules/rules-open.css', 'src/rules/rules.css'].map(read).join('\n')
// The phone, the distance view and the table screen ship their own sheets and not the editor's;
// the help pattern's two sheets are in both lists, because the box is the same box on either
// screen (L32's addendum, #305).
const phoneCss = ['src/help.css', 'src/help-box.css', 'src/table/table.css', 'src/player/player.css', 'src/buttons.css', 'src/table/keyboard.css', 'src/rules/rules-open.css', 'src/rules/rules.css', 'src/table/texture.css'].map(read).join('\n')
...
// The phone, the distance view and the table screen (L32's addendum, #305).
//
// The decision was made on one column of the prototype's table: the box covers **0 %** of what
// the hand reaches, where a sheet from the bottom covered 68 % and a full screen 100 %. A help
// surface lying over four cards out of five and all three buttons does not meet «hjälpytor
// blockerar inte nödvändiga spelhandlingar», so what is asserted here is that property and not a
// pixel: no card and no action button is under the box, and the box is inside the screen.
//
// The price the decision accepts is that the cross stands near the top, out of the thumb's reach;
// what makes it bearable is that a press outside closes it, and «outside» on a phone is nearly
// the whole screen. That half is jsdom's to check (`player-page.test.tsx`), because it is
// behaviour and not layout.
describe('the same box on the phone, the distance view and the table screen (#305)', () => {
  ...
  type Laid = { ask: Rect; box: Rect; under: string | null; plays: { what: string; rect: Rect }[] }
  ...
  // cannot have covered.
  async function laid(html: string, must: string, view = { w: 390, h: 844 }): Promise<Laid> {
  ...
  // The table screen, where the same pattern holds at the other end of the scale: the box is the
  // same 260 px, the column beside the felt is narrow, and what it must not cover is the code and
  // the square the room joins by — the very thing it is about.
