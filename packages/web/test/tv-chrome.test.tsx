...
// The same box on the table screen (L32, #305): the line beside the code is four words, and how
// a phone becomes a hand is the rest of it.
describe('the table screen’s own help (L32, #305)', () => {
  it('keeps the code on the surface and what joining means behind a question mark', () => {
    const { view, log } = buildScene()
    render(
      <TvChrome view={view(null)} activity={log.map(projectActivity)} roomCode="KX7P">
        <div />
    ...
    const box = () => screen.queryByRole('dialog', { name: 'att ansluta' })
    expect(box()).toBeNull()
    fireEvent.pointerEnter(ask)
    expect(box()).toBeNull()

    fireEvent.click(ask)
    expect(box()!.textContent).toMatch(/rumskoden/i)
