...
const shelf = (zones: ZoneView[] | null) =>
  render(<RuleShelf rules={book} placement="table" startOpen {...(zones ? { live: { zones } } : {})} />)

describe('the living number beside a tagged zone (#226)', () => {
  it('shows the count the reader’s own projection reports, beside the name and not inside the clause', () => {
    shelf([read('discard', 'Kasthög', ['a', 'b', 'c'])])
  ...
  it('fills the badge where the book may read the zone and leaves it hollow where it may only count', () => {
    shelf([read('discard', 'Kasthög', ['a', 'b', 'c']), counted('draw', 'Draghög', 18)])
  ...
  it('says out loud what the badge only shapes, in form A’s own words', () => {
    shelf([counted('draw', 'Draghög', 18), read('discard', 'Kasthög', ['a', 'b', 'c'])])
  ...
  it('draws a card reference exactly as a book with no table under it draws it', () => {
    const { container } = shelf([counted('draw', 'Draghög', 18), read('discard', 'Kasthög', [])])
    const atTable = container.querySelector('[data-ref="vargen"]')!.outerHTML
    const withoutTable = shelf(null).container.querySelector('[data-ref="vargen"]')!.outerHTML
    expect(atTable).toBe(withoutTable)
    expect(atTable).not.toContain('byd-rules-tally')
  })

  it('leaves every tag bare when no table is running, so the tag stands for the name', () => {
    const { container } = shelf(null)
  ...
  it('gives no number to a zone this table has not got', () => {
    shelf([read('discard', 'Kasthög', ['a'])])
  ...
  it('keeps the living number out of what the question box searches', () => {
    shelf([counted('draw', 'Draghög', 18)])
