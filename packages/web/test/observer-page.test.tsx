...
// The same box on the distance view (L32, #305): what she may do and what everyone else is told
// is more than the handle's one line can carry, and the banner that says what she sees stays
// where it is — visibility information never moves behind a question mark.
describe('the observer’s own help (L32, #305)', () => {
  it('keeps the banner on the surface and the rest behind a question mark that hover does not open', async () => {
    const id = await createSession(run)
    history.replaceState(null, '', `/observe?session=${id}&name=Eva&token=${await admit(run, id, null, 'Eva')}&server=${encodeURIComponent(run.url)}`)
    render(<ObserverPage />)
    ...
    fireEvent.click(ask)
    expect(box()!.textContent).toMatch(/aldrig röra ett kort/i)
