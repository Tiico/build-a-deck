...
// A top lying face-down still shows its back to the whole table, as it does on a real one, so
// the zone carries `back`: the texture hash of that card's back (#14, #313) — the hash and
// nothing else, never the card's identity or its front. Absent when the pile is empty, when the
// session has no textures, and when the top is face-up and named instead.
